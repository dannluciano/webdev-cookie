...
test('cadastro duplica bloqueado com status 409', async () => {
  ...
  try {
    const firstResponse = await createUser(baseUrl, 'ana@example.com', '12345678');
    const firstHtml = await firstResponse.text();
    assert.equal(firstResponse.status, 200);
    assert.match(firstHtml, /Usuário criado/);

    const secondResponse = await createUser(baseUrl, 'ana@example.com', '12345678');
...
test('login inválido retorna 401 para usuário inexistente e senha incorreta', async () => {
  ...
  try {
    const missingUserResponse = await loginUser(baseUrl, 'ghost@example.com', '12345678');
    ...
    const wrongPasswordResponse = await loginUser(baseUrl, 'duda@example.com', '87654321');
...
test('login válido redireciona para dashboard e libera admin', async () => {
  ...
  try {
    ...
    const loginResponse = await loginUser(baseUrl, 'bia@example.com', '12345678');
...
test('home autenticada troca a navegação para rotas privadas', async () => {
  ...
  try {
    ...
    const loginResponse = await loginUser(baseUrl, 'eve@example.com', '12345678');
...
test('logout remove a sessão e volta a bloquear dashboard', async () => {
  ...
  try {
    ...
    const loginResponse = await loginUser(baseUrl, 'cai@example.com', '12345678');
    const authCookie = loginResponse.headers.get('set-cookie').split(';', 1)[0];
...
test('admin mostra sessão anônima e timestamp ausente como N/A', async () => {
  ...
  try {
    ...
    const loginResponse = await loginUser(baseUrl, 'fred@example.com', '12345678');
    const authCookie = loginResponse.headers.get('set-cookie').split(';', 1)[0];
...
test('admin preserva views existentes e ignora segmentos inválidos de cookie', async () => {
  ...
  try {
    ...
    const loginResponse = await loginUser(baseUrl, 'gina@example.com', '12345678');
    const authCookie = loginResponse.headers.get('set-cookie').split(';', 1)[0];
